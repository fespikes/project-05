...
export const wsApiServiceUrl = new InjectionToken('wsApiServiceUrl');
...
export class WsApiService {
  ...
  constructor(
    ...
    @Optional() @Inject(wsApiServiceUrl) private apiUrl,
  ...
  formatErrors(responseError: HttpErrorResponse) {
    ...
    this.message.error(data.error); // TODO
  ...
  makeUrl(url) {
    ...
    return this.join(this.apiUrl || environment.apiUrl, url);
  }
